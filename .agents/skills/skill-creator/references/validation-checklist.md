...
- [ ] Env vars: respects hierarchy `process.env` > `%USERPROFILE%/.claude/skills/${SKILL}/.env` (global) > `%USERPROFILE%/.claude/skills/.env` (global) > `%USERPROFILE%/.claude/.env` (global) > `%USERPROFILE%/.claude/skills/${SKILL}/.env` (cwd) > `%USERPROFILE%/.claude/skills/.env` (cwd) > `%USERPROFILE%/.claude/.env` (cwd)
