...
SKILL_DIR=""
if [ -d "%USERPROFILE%/.claude/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="%USERPROFILE%/.claude/skills/chrome-devtools/scripts"
elif [ -d "%USERPROFILE%/.claude/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="%USERPROFILE%/.claude/skills/chrome-devtools/scripts"
fi
...
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output %USERPROFILE%/.claude/chrome-devtools/snapshots/page.yaml
```
...
mkdir -p %USERPROFILE%/.claude/chrome-devtools/snapshots

# Capture and store with timestamp
SESSION="$(date +%Y%m%d-%H%M%S)"
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output %USERPROFILE%/.claude/chrome-devtools/snapshots/$SESSION.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
All in `%USERPROFILE%/.claude/skills/chrome-devtools/scripts/`:
...
```bash
# Basic screenshot
node "$SKILL_DIR/screenshot.js" --url https://example.com --output %USERPROFILE%/.claude/chrome-devtools/screenshots/page.png

# Full page
node "$SKILL_DIR/screenshot.js" --url https://example.com --output %USERPROFILE%/.claude/chrome-devtools/screenshots/page.png --full-page true

# Specific element
node "$SKILL_DIR/screenshot.js" --url https://example.com --selector ".main-content" --output %USERPROFILE%/.claude/chrome-devtools/screenshots/element.png
```
...
node "$SKILL_DIR/screenshot.js" --url https://example.com --output %USERPROFILE%/.claude/chrome-devtools/screenshots/page.png --max-size 3

# Disable compression
node "$SKILL_DIR/screenshot.js" --url https://example.com --output %USERPROFILE%/.claude/chrome-devtools/screenshots/page.png --no-compress
```
...
SESSION="$(date +%Y%m%d-%H%M%S)"
mkdir -p %USERPROFILE%/.claude/chrome-devtools/logs/$SESSION

# Capture and store
node "$SKILL_DIR/console.js" --url https://example.com --duration 10000 > %USERPROFILE%/.claude/chrome-devtools/logs/$SESSION/console.json
node "$SKILL_DIR/network.js" --url https://example.com > %USERPROFILE%/.claude/chrome-devtools/logs/$SESSION/network.json

# View errors
jq '.messages[] | select(.type=="error")' %USERPROFILE%/.claude/chrome-devtools/logs/$SESSION/console.json
```
...
node "$SKILL_DIR/screenshot.js" --output %USERPROFILE%/.claude/skills/chrome-devtools/screenshots/debug.png
