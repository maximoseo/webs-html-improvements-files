...
def create_auth_token(username: str) -> str:
    ...
    payload = _b64url_enc(json.dumps({'sub': username, 'exp': int(time.time()) + _TOKEN_EXPIRY, 'iat': int(time.time())}).encode())
    sig_input = f'{header}.{payload}'.encode()
    sig = _b64url_enc(_hmac_module.new(_JWT_SECRET.encode(), sig_input, hashlib.sha256).digest())
...
def verify_auth_token(token: str):
    ...
    try:
        ...
        sig_input = f'{header}.{payload_part}'.encode()
        expected_sig = _b64url_enc(_hmac_module.new(_JWT_SECRET.encode(), sig_input, hashlib.sha256).digest())
        if not _hmac_module.compare_digest(expected_sig, sig):
            return None
        data = json.loads(_b64url_dec(payload_part))
...
def _get_request_token(handler) -> str:
    ...
    for part in cookie.split(';'):
        ...
        if part.startswith('dashboard_token='):
            return part[16:].strip()
    return ''


def _check_auth(handler) -> bool:
    """Returns True if request is authenticated."""
    token = _get_request_token(handler)
    if not token:
        return False
    return verify_auth_token(token) is not None


def _send_401(handler):
    """Send 401 JSON response."""
...
def _save_login_to_supabase(username: str, ip: str, event: str = 'login'):
    """Save login event to Supabase dashboard_logins table."""
    try:
        pdata = json.dumps({
            'username': username,
            'event': event,
        ...
        req = urllib.request.Request(
            f'{SUPABASE_URL_AUTH}/rest/v1/dashboard_logins',
            data=pdata,
            headers={
                'Content-Type': 'application/json',
                'apikey': SUPABASE_ANON_KEY_AUTH,
                'Authorization': f'Bearer {SUPABASE_ANON_KEY_AUTH}',
        ...
        logging.warning(f'[auth] supabase login save failed: {e}')


def _save_login_to_obsidian(username: str, ip: str, event: str = 'login'):
    """Append login event to Obsidian vault login-log.md."""
    try:
        ts = time.strftime('%Y-%m-%d %H:%M:%S UTC', time.gmtime())
        note_line = f'- [{ts}] **{event}** | user: `{username}` | ip: `{ip}`\n'
        path = 'dashboard-auth/login-log.md'
        existing = '# Dashboard Auth Log\n\n'
        try:
            get_req = urllib.request.Request(
                f'{OBSIDIAN_URL_AUTH}/vault/{path}',
                headers={
                    'Authorization': f'Bearer {OBSIDIAN_KEY_AUTH}',
                    'ngrok-skip-browser-warning': 'true',
                },
                method='GET',
            )
            existing = urllib.request.urlopen(get_req, timeout=5).read().decode('utf-8', 'replace')
        except Exception:
            pass
        content = existing + note_line
        ...
        logging.warning(f'[auth] obsidian login save failed: {e}')


# ===== LOGIN PAGE =====
_LOGIN_HTML_PATH = Path(__file__).resolve().parent / 'login-page.html'
def _get_login_html() -> bytes:
    try:
        return _LOGIN_HTML_PATH.read_bytes()
    except Exception:
        return b'<h1>Login page not found</h1>'
...
RADAR_SOURCES = [
    ...
    # GitHubAwesome.com - curated daily trending repos
    {'url': 'https://githubawesome.com/', 'title': 'GitHubAwesome - Daily Trending', 'type': 'html_page', 'quality': 0.92},
    {'url': 'https://githubawesome.com/page/2', 'title': 'GitHubAwesome Page 2', 'type': 'html_page', 'quality': 0.90},
    {'url': 'https://githubawesome.com/page/3', 'title': 'GitHubAwesome Page 3', 'type': 'html_page', 'quality': 0.88},
    # Design & CSS skills
    {'url': 'https://raw.githubusercontent.com/VoltAgent/awesome-design-md/main/README.md', 'title': 'Awesome Design Systems (55 companies)', 'type': 'awesome_list', 'quality': 0.95},
    {'url': 'https://raw.githubusercontent.com/Manavarya09/design-extract/main/README.md', 'title': 'Design Token Extractor', 'type': 'github_repo', 'quality': 0.90},
    # AI Coding Skills & Agents
    {'url': 'https://raw.githubusercontent.com/codejunkie99/agentic-stack/main/README.md', 'title': 'Agentic Stack - AI Coding Tools', 'type': 'github_repo', 'quality': 0.90},
    {'url': 'https://raw.githubusercontent.com/hexiecs/talk-normal/main/README.md', 'title': 'Talk Normal - Claude Token Compression', 'type': 'github_repo', 'quality': 0.92},
    {'url': 'https://raw.githubusercontent.com/browser-use/browser-harness/main/README.md', 'title': 'Browser Harness - JS Automation', 'type': 'github_repo', 'quality': 0.88},
    {'url': 'https://raw.githubusercontent.com/garrytan/gbrain/main/README.md', 'title': 'GBrain - AI Knowledge System', 'type': 'github_repo', 'quality': 0.85},
    {'url': 'https://raw.githubusercontent.com/safishamsi/graphify/main/README.md', 'title': 'Graphify - Codebase Knowledge Graph', 'type': 'github_repo', 'quality': 0.85},
    # Awesome lists for agent skills
    {'url': 'https://raw.githubusercontent.com/f/awesome-chatgpt-prompts/main/README.md', 'title': 'Awesome ChatGPT Prompts', 'type': 'awesome_list', 'quality': 0.88},
    {'url': 'https://raw.githubusercontent.com/steven2358/awesome-generative-ai/main/README.md', 'title': 'Awesome Generative AI', 'type': 'awesome_list', 'quality': 0.90},
    {'url': 'https://raw.githubusercontent.com/filipecalegario/awesome-generative-ai-tools/main/README.md', 'title': 'Awesome GenAI Tools', 'type': 'awesome_list', 'quality': 0.88},
    {'url': 'https://raw.githubusercontent.com/aimerou/awesome-ai-papers/main/README.md', 'title': 'Awesome AI Papers', 'type': 'awesome_list', 'quality': 0.85},
]
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # --- AUTH: public static assets (skip auth for .js .css .ico .png .svg .woff) ---
        _static_ext = ('.js', '.css', '.ico', '.png', '.svg', '.woff', '.woff2', '.ttf', '.eot', '.map')
        _is_static = any(parsed.path.endswith(ext) for ext in _static_ext)

        # --- AUTH: require token for all non-public routes ---
        if not _is_static and not _check_auth(self):
            # API calls get 401, browser GET requests get redirect
            if parsed.path.startswith('/api/'):
                return _send_401(self)
            else:
                return _redirect_login(self)
        ...
        if parsed.path == '/api/task-notifications':
            try:
                overdue = _get_overdue_tasks()
                due_today = _get_due_today_tasks()
                return json_response(self, 200, {
                    'ok': True,
                    'overdue': overdue,
                    'due_today': due_today,
        ...
        if parsed.path == '/api/login':
            try:
                ...
                if username == _AUTH_USER and password == _AUTH_PASS:
                    ...
                    threading.Thread(target=_save_login_to_supabase, args=(username, ip, 'login'), daemon=True).start()
                    threading.Thread(target=_save_login_to_obsidian, args=(username, ip, 'login'), daemon=True).start()
                    return json_response(self, 200, {'ok': True, 'token': token})
                else:
                    return json_response(self, 401, {'ok': False, 'error': 'Invalid credentials'})
            except Exception as e:
                return json_response(self, 500, {'ok': False, 'error': str(e)})

        # --- AUTH: password reset endpoint (no auth required) ---
        if parsed.path == '/api/reset-password':
            try:
                cl = int(self.headers.get('Content-Length', 0))
                body = json.loads(self.rfile.read(cl)) if cl else {}
                email = body.get('email', 'service@maximo-seo.com').strip()
                temp_pass = _gen_temp_password()
                # Send via Resend
                resend_payload = json.dumps({
                    'from': 'noreply@maximo-seo.com',
                    'to': [email],
                    'subject': 'Dashboard Password Reset',
                    'html': f'<p>Your temporary password is: <strong>{temp_pass}</strong></p><p>Please update it after login.</p>',
                }).encode('utf-8')
                req = urllib.request.Request(
                    ...
                    headers={
                        'Content-Type': 'application/json',
                        'Authorization': f'Bearer {RESEND_API_KEY_AUTH}',
                    },
                    method='POST',
                )
                urllib.request.urlopen(req, timeout=10)
                ip = self.client_address[0]
                threading.Thread(target=_save_login_to_supabase, args=(_AUTH_USER, ip, 'reset'), daemon=True).start()
                threading.Thread(target=_save_login_to_obsidian, args=(_AUTH_USER, ip, 'reset'), daemon=True).start()
                return json_response(self, 200, {'ok': True, 'message': 'Reset email sent'})
            except Exception as e:
                return json_response(self, 500, {'ok': False, 'error': str(e)})

        # --- AUTH: require token for all other POST routes ---
    ...
    def serve_static(self, path):
        ...
        if parsed.path == '/api/delete-agent':
            ...
            try:
                gh_token = os.getenv('GITHUB_TOKEN', '')
                gh_repo = 'maximoseo/webs-html-improvements-files'
                agent_path = f"{project}/{agent}"
                headers = {
                ...
                list_url = f'https://api.github.com/repos/{gh_repo}/contents/{agent_path}'
                agent_files = _gh_get(list_url)
                if not isinstance(agent_files, list):
                    return json_response(self, 404, {'ok': False, 'error': f'Agent path not found: {agent_path}'})
                moved = 0
                for file_obj in agent_files:
                    if file_obj.get('type') != 'file':
                        continue
                    src_path = file_obj['path']
                    file_content = _gh_get(f'https://api.github.com/repos/{gh_repo}/contents/{src_path}')
                    content_b64 = file_content.get('content', '').replace('\n', '')
                    sha = file_content.get('sha', '')
                    dst_path = f'_deleted/{project}/{agent}/{file_obj["name"]}'
                    # Create file in _deleted/
                    create_data = _json.dumps({
                        'message': f'[dashboard] soft-delete {src_path}',
                        'content': content_b64
                    }).encode('utf-8')
                    create_req = _ureq.Request(
                        f'https://api.github.com/repos/{gh_repo}/contents/{dst_path}',
                        data=create_data,
                        headers={**headers, 'Content-Type': 'application/json'},
                        method='PUT'
                    )
                    _ureq.urlopen(create_req, timeout=15)
                    # Delete original
                    del_data = _json.dumps({'message': f'[dashboard] delete {src_path}', 'sha': sha}).encode('utf-8')
                    del_req = _ureq.Request(
                        f'https://api.github.com/repos/{gh_repo}/contents/{src_path}',
                        data=del_data,
                        headers={**headers, 'Content-Type': 'application/json'},
                        method='DELETE'
                    )
                    _ureq.urlopen(del_req, timeout=15)
                    moved += 1
                return json_response(self, 200, {'ok': True, 'moved': moved, 'destination': f'_deleted/{project}/{agent}'})
            except Exception as e:
                logging.error(f'[delete-agent] {e}')
                return json_response(self, 500, {'ok': False, 'error': str(e)[:200]})
        return json_response(self, 404, {'ok': False, 'error': 'Not found'})
...
# ============================================================
# TASK DUE DATE NOTIFICATION SYSTEM
# Runs daily at 05:00 UTC (08:00 Israel time)
# Sends email via Resend + Telegram notification
# ============================================================

import datetime as _dt

_task_notify_lock = threading.Lock()
_last_task_notify_date = None

def _get_overdue_tasks():
    """Fetch overdue tasks from Supabase."""
    cfg = supabase_radar_config()
    if not cfg:
        return []
    try:
        ...
        hdrs = _radar_sb_headers()
        url = cfg['url'] + f"/rest/v1/tasks?due=lt.{today}&status=neq.done&status=neq.Done&order=due.asc"
        data = fetch_json(url, headers=hdrs, timeout=10)
        if isinstance(data, list):
            return data
    except Exception as e:
        logging.error(f'[task-notify] fetch overdue tasks: {e}')
    return []

def _get_due_today_tasks():
    ...
    try:
        today = _dt.date.today().isoformat()
        hdrs = _radar_sb_headers()
        url = cfg['url'] + f"/rest/v1/tasks?due=eq.{today}&status=neq.done&status=neq.Done&order=priority.desc"
        data = fetch_json(url, headers=hdrs, timeout=10)
        if isinstance(data, list):
            return data
    except Exception as e:
        logging.error(f'[task-notify] fetch due today tasks: {e}')
    return []

def _send_task_notification():
    """Send task due date notification via email and Telegram."""
    overdue = _get_overdue_tasks()
    due_today = _get_due_today_tasks()

    if not overdue and not due_today:
        logging.info('[task-notify] No overdue or due-today tasks. Skipping notification.')
        return
    ...
    def priority_emoji(p):
        return {'high': '\U0001f534', 'medium': '\U0001f7e1', 'low': '\U0001f7e2'}.get((p or '').lower(), '\u26aa')

    email_body = (
        "<html><body style='font-family:sans-serif;background:#0a0a0a;color:#eee;padding:24px'>"
        f"<h2 style='color:#a78bfa'>\U0001f4cb Daily Task Report \u2014 {today_str}</h2>"
    )

    if overdue:
        email_body += f"<h3 style='color:#f87171'>\U0001f6a8 Overdue Tasks ({len(overdue)})</h3><ul>"
        for t in overdue:
            email_body += (f"<li>{priority_emoji(t.get('priority'))} "
                           f"<strong>{t.get('title','?')}</strong> \u2014 "
                           f"Due: {t.get('due','?')} [{t.get('project','general')}]</li>")
        email_body += "</ul>"

    if due_today:
        email_body += f"<h3 style='color:#fbbf24'>\u23f0 Due Today ({len(due_today)})</h3><ul>"
        for t in due_today:
            email_body += (f"<li>{priority_emoji(t.get('priority'))} "
                           f"<strong>{t.get('title','?')}</strong> [{t.get('project','general')}]</li>")
        email_body += "</ul>"

    email_body += "<p style='color:#888;font-size:12px'>Sent from HTML Redesign Dashboard</p></body></html>"

    # Send email via Resend
    resend_key = os.getenv('RESEND_API_KEY', '')
    email_to = os.getenv('TASK_NOTIFY_EMAIL', os.getenv('RADAR_EMAIL_TO', 'service@maximo-seo.com'))

    if resend_key:
        try:
            subject = f"\U0001f4cb Tasks: {len(overdue)} overdue, {len(due_today)} due today \u2014 {today_str}"
            payload = {
                'from': os.getenv('RADAR_EMAIL_FROM', 'onboarding@resend.dev'),
                'to': [email_to],
                'subject': subject,
                'html': email_body
            }
            ...
            req = urllib.request.Request(resend_url, data=body,
                headers={'Authorization': f'Bearer {resend_key}', 'Content-Type': 'application/json'},
                method='POST')
            with urllib.request.urlopen(req, timeout=15) as r:
                logging.info(f'[task-notify] Email sent to {email_to}: {r.status}')
        except Exception as e:
            logging.error(f'[task-notify] Email failed: {e}')
    ...
    if tg_token and tg_chat:
        try:
            lines = [f'\U0001f4cb *Daily Task Report \u2014 {today_str}*']
            if overdue:
                lines.append(f'\n\U0001f6a8 *Overdue ({len(overdue)}):*')
                for t in overdue[:5]:
                    lines.append(f"  {priority_emoji(t.get('priority'))} {t.get('title','?')} \u2014 {t.get('due','?')}")
                if len(overdue) > 5:
                    lines.append(f'  ... and {len(overdue)-5} more')
            if due_today:
                lines.append(f'\n\u23f0 *Due Today ({len(due_today)}):*')
                for t in due_today[:5]:
                    lines.append(f"  {priority_emoji(t.get('priority'))} {t.get('title','?')}")
                if len(due_today) > 5:
                    lines.append(f'  ... and {len(due_today)-5} more')
            lines.append('\n\U0001f517 https://html-redesign-dashboard.maximo-seo.ai/')

            msg = '\n'.join(lines)
            tg_payload = {'chat_id': tg_chat, 'text': msg, 'parse_mode': 'Markdown'}
            tg_url = f'https://api.telegram.org/bot{tg_token}/sendMessage'
            tg_body = json.dumps(tg_payload).encode('utf-8')
            tg_req = urllib.request.Request(tg_url, data=tg_body,
                headers={'Content-Type': 'application/json'}, method='POST')
            with urllib.request.urlopen(tg_req, timeout=10) as r:
                logging.info(f'[task-notify] Telegram sent: {r.status}')
        except Exception as e:
            logging.error(f'[task-notify] Telegram failed: {e}')

def _task_notification_scheduler():
    """Background thread: check every minute if it's time to send task notification."""
    global _last_task_notify_date
    while True:
        try:
            now_utc = _dt.datetime.utcnow()
            today = now_utc.date()
            # Send at 05:00 UTC = 08:00 Israel time
            if now_utc.hour == 5 and now_utc.minute == 0:
                with _task_notify_lock:
                    if _last_task_notify_date != today:
                        _last_task_notify_date = today
                        threading.Thread(target=_send_task_notification, daemon=True).start()
        except Exception as e:
            logging.error(f'[task-notify-scheduler] {e}')
        time.sleep(60)


threading.Thread(target=_radar_scheduler_loop, daemon=True).start()
threading.Thread(target=_task_notification_scheduler, daemon=True).start()
