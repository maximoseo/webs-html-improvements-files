...
Tests:
  1. POST /api/auth/login with admin creds → 200 + Set-Cookie: dash_auth
  2. POST /api/login (alias) → 200 + Set-Cookie: dash_auth
  3. POST with bad creds → 401 (invalid_credentials)
  4. Rate limit after 11 bad attempts → 429
  5. GET / without cookie → 302 to /login (when gate enforced)
  6. GET / with cookie → 200 (dashboard)
...
def main(base: str) -> int:
    ...
    # 1. /api/auth/login — primary endpoint
    ...
    if code == 200 and data.get("ok") and "Set-Cookie" in {k.title() for k in headers}:
        print("PASS 1  /api/auth/login with admin creds → 200 + Set-Cookie")
    else:
        failures.append(f"FAIL 1  /api/auth/login returned {code} {data}; headers={list(headers)}")
    ...
    if code == 200 and data.get("ok"):
        print("PASS 2  /api/login alias → 200")
    else:
        failures.append(f"FAIL 2  /api/login returned {code} {data}")

    # 3. Bad creds → 401
    code, _, body = _request(base, "/api/auth/login", "POST",
                             {"user": "admin", "password": "definitely-wrong-xyz"})
    data = json.loads(body or b"{}")
    if code == 401 and data.get("error") == "invalid_credentials":
        print("PASS 3  Bad creds → 401 invalid_credentials")
    else:
        failures.append(f"FAIL 3  Bad creds returned {code} {data}")

    # 4. Rate limit — 11 bad attempts, expect a 429 somewhere in the last 2.
    got_429 = False
    for _ in range(12):
        ...
        if code == 429:
            ...
            break
    if got_429:
        print("PASS 4  Rate limit → 429 returned")
    else:
        failures.append("FAIL 4  Rate limit did not trigger within 12 bad attempts")
